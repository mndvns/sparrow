(->
    ...
    validate: ->
      ...
      if not @isPersisted()
        if (cons._limit - cons.mine().count() <= 0)
          console.log("CONS", cons._limit, cons.mine().fetch() )
  ...
  class App.Model.Location extends Model
    @_type: "Location"
  ...
  class App.Model.Tag extends Model
    @_type: "Tag"
  ...
  class App.Model.Offer extends Model
    @_type: "Offer"
